...
import { FiCalendar, FiClock, FiMapPin, FiDollarSign, FiCheck, FiX, FiFilter } from 'react-icons/fi';
    ...
    <div className="container mx-auto px-4 py-8">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-6 space-y-4 md:space-y-0">
        <div className="flex space-x-1 bg-gray-100 p-1 rounded-lg">
          <button
            className={`px-4 py-2 rounded-md ${activeTab === 'upcoming' ? 'bg-white shadow-sm' : 'text-gray-600'}`}
            onClick={() => setActiveTab('upcoming')}
          >
            {t('worker.upcomingJobs')}
          </button>
          <button
            className={`px-4 py-2 rounded-md ${activeTab === 'completed' ? 'bg-white shadow-sm' : 'text-gray-600'}`}
              ...
              <div className="flex flex-col md:flex-row justify-between">
                  ...
                  {job.notes && (
                    <div className="mt-4 p-3 bg-gray-50 rounded-md">
                      <p className="text-sm text-gray-700">
                        <span className="font-medium">{t('worker.customerNotes')}:</span> {job.notes}
                      </p>
                    </div>
                  )}
                </div>
                ...
                <div className="mt-6 md:mt-0 md:ml-6 flex flex-col items-end justify-between">
                  <div className="text-right">
                    <p className="text-sm font-medium text-gray-900">{t('worker.customer')}</p>
                    <p className="text-base">{job.customerName}</p>
                    {job.customerPhone && <p className="text-sm text-gray-600">{job.customerPhone}</p>}
                  </div>
