...
export const customerProfileSchema = yup
  .object({
    ...
    preferredLanguage: yup
      .string()
      .required("customer.errors.requiredLanguage"),
  })
  .required();

export const serviceBookingSchema = yup
  .object({
    serviceId: yup.string().required("booking.errors.requiredService"),
    workerId: yup.string().required("booking.errors.requiredWorker"),
    date: yup
      .date()
      .required("booking.errors.requiredDate")
      .min(new Date(), "booking.errors.pastDate"),
...
export const paymentSchema = yup
  .object({
    ...
    cardNumber: yup.string().when("paymentMethod", {
      ...
      then: (schema) =>
        schema
          .required("payment.errors.requiredCardNumber")
          .matches(/^\d{16}$/, "payment.errors.invalidCardNumber"),
    ...
    cardExpiry: yup.string().when("paymentMethod", {
      ...
      then: (schema) =>
        schema
          .required("payment.errors.requiredCardExpiry")
          .matches(
            /^(0[1-9]|1[0-2])\/\d{2}$/,
            "payment.errors.invalidCardExpiry"
          ),
      otherwise: (schema) => schema,
    }),
    cardCvv: yup.string().when("paymentMethod", {
      ...
      then: (schema) =>
        schema
          .required("payment.errors.requiredCardCvv")
          .matches(/^\d{3,4}$/, "payment.errors.invalidCardCvv"),
      otherwise: (schema) => schema,
    }),
    upiId: yup.string().when("paymentMethod", {
      ...
      then: (schema) =>
        schema
          .required("payment.errors.requiredUpiId")
          .matches(/^[\w.-]+@[\w.-]+$/, "payment.errors.invalidUpiId"),
      otherwise: (schema) => schema,
    }),
  })
  .required();
